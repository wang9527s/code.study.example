...
public:
    Client(QObject * parent=nullptr)
        :QObject (parent)
      , m_inter(new QDBusInterface(ServiceName, ServicePath,ServiceInterface,QDBusConnection::sessionBus()))
    {
        connect(m_inter,SIGNAL(sigProgress(int ,QString ))
    ...
    QString getResult(){
        QDBusPendingReply<QString> reply =
                m_inter->asyncCall(QLatin1String("result"));
